...
public class TreeQuerySet {
	...
	public static void getTree() throws DatabaseException, InterruptedException {
		...
		try {
			...
			ps = con.prepareStatement(
					"select * from sensor as s join room as r join floor as f join building as b join area as a join city as c on s.ID_room=r.ID and r.ID_floor=f.ID and f.ID_building=b.ID and b.ID_area=a.ID and a.ID_city=1;");
			rs = ps.executeQuery();
			...
			while (rs.next()) {
				...
				// System.out.println(rs.getInt("s.ID_sensor")+" "+rs.getBoolean("s.status")+"
				// "+rs.getInt("s.type")+" "+rs.getShort("s.threshold")+rs.getInt("s.ID_room"));
				getSensors();
			}
			// Cache.setSensorMap(sensors);
			Cache.setRoomMap(rooms);
			Cache.setBuildingMap(buildings);
			...
			/*
			for (Area area : city.getSubs().values()) {
				System.out.println("|-- AREA :" + area.getName());
				// System.out.println("|__");
				for (Building building : area.getSubs().values()) {
					System.out.println("    |-- BUILDING :" + building.getStreet());
					// System.out.println(" |__");
				}
			}*/
	...
	public static void getSensors() {
		try {
			Sensor s = new Sensor(rs.getInt("s.ID_sensor"), rs.getBoolean("s.status"), rs.getInt("s.type"),
					rs.getShort("s.threshold"), 0, rs.getInt("s.ID_room"), getRoom());
	...
	public static Building getBuilding() {
		...
		try {
			...
			b = new Building(rs.getInt("b.ID"), rs.getString("b.street"), rs.getInt("b.number"), rs.getInt("b.ID_Area"),
					getArea());
